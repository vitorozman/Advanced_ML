...
def pretvornik_ur(niz):
    """
    Sprejme niz oblike 11:10:10 in vrne ure kot stevilo
    """
    return int(niz[:2])


def razsiri_z_hours(business_ids):
    tbl_hours = PODATKI['hours']
    dnevi = tbl_hours['day'].unique()
    new_col = []
    for day in dnevi:
        new_col.append(day + "_open")
        new_col.append(day + "_close")
    tbl_hours["open time"] = tbl_hours["open time"].apply(pretvornik_ur)
    tbl_hours["close time"] = tbl_hours["close time"].apply(pretvornik_ur)
    ...
    for _, vrsta in tbl_hours.iterrows():
        biznis = vrsta["business_id"]
        dan = vrsta["day"]
        odpr_cas = vrsta["open time"]
        zapr_cas = vrsta["close time"]
        new_tbl.at[biznis, dan + "_open"] = odpr_cas
        new_tbl.at[biznis, dan + "_close"] = zapr_cas

    return np.array(new_tbl)
...
def razsiri_z_attributes(business_ids):
    tbl_att = PODATKI['attributes']
    imena_att = list(tbl_att['name'].unique())
    # '9999' damo da se da pretvoriti v stevilsko in se ne pojavi v nasih tbl
    new_tbl = pd.DataFrame(
        index=business_ids,
        columns=imena_att,
        data="9999"
    )
    for _, vrsta in tbl_att.iterrows():
        biznis = vrsta["business_id"]
        ime_att = vrsta["name"]
        vrednost = vrsta["value"]
        new_tbl.at[biznis, ime_att] = vrednost
    for stolpec in new_tbl.columns:
        num_vrednosti = pd.to_numeric(new_tbl[stolpec], errors="coerce")
        if all(num_vrednosti.notnull()):
            new_tbl[stolpec] = num_vrednosti
    return pretvori_vse_one_hot(new_tbl, imena_att, drop=["9999"])
...
for podseznam in generiraj_vse_neprazne_podsezname(n_blokov):
    uporabljeni_bloki = [imena_blokov[i] for i in podseznam]
    print()
    print(f"Rezultati za bloke {uporabljeni_bloki}:")
    xs = np.block([vsi_bloki[i] for i in podseznam])
